...
contract TokenFactory {
    ...
    function getId(address _creator, string memory _name, string memory _symbol, uint _limit, uint8 _decimals) internal pure returns(bytes32 id) {
        return keccak256(abi.encodePacked(_creator, _name, _symbol, _limit, _decimals));
    }

    event TokenCreated(address token, address creator, string name, string symbol, uint limit, uint8 decimals);

    function _createToken(address _creator, string memory _name, string memory _symbol, uint _limit, uint8 _decimals) internal returns (address tokenContract) {
        ...
        tokenContract = address(new Token{salt: getId(_creator, _name, _symbol, _limit, _decimals)}());
        ...
        emit TokenCreated(tokenContract, _creator, _name, _symbol, _limit, _decimals);
    }
    ...
    /**
        ...
        @return tokenContract The address of the newly created Token contract
      */
    function createOwnerlessToken(string memory _name, string memory _symbol, uint _limit, uint8 _decimals) external returns (address tokenContract) {
